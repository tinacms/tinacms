...
Once the PR is merged and has completed it's actions, you can install the changes by installing the @dev version of the package. So if there were changes to `@strivemath/tinacms-graphql` merged into `main`, you can test them out by running `pnpm add @strivemath/tinacms-graphql@beta`.

...
import { NonEditableVideo } from './non-editable-video'
...
interface VideoRenderChildrenProps {
  src?: string
}

export type VideoRenderChildren = (
  props: VideoRenderChildrenProps
) => React.ReactNode
 ...
 * @alias InlineVideo
 */
export const InlineVideoField = InlineVideo
...
export function InlineVideo(props: InlineVideoProps) {
  ...
  return (
    <InlineField name={props.name} parse={props.parse}>
      {({ input, form }) => {
        if (cms.enabled) {
          return <EditableVideo {...props} input={input} form={form} />
        }

...
const listReplace = (listType: ListType): ElementTransformer['replace'] => {
  return (parentNode, children, match) => {
    ...
    // Assumes the parent nodes are Flow content, we want to basically reparent them
    // the list -> list item. I guess `parentNode.replace` / `parentNode.remove` from above
    // Doesn't actually happen until the commit phase, which allows us to continue to use
    // it here
    listItem.select(0, 0)
    if (children.length) {
...
export const QUOTE: ElementTransformer = {
  ...
  replace: (parentNode, children, _match, isImport) => {
    ...
    const node = $createTinaQuoteNode()
    parentNode.replace(node)
    // Assumes the parent nodes are Flow content, we want to basically reparent them
    // the list -> list item. I guess `parentNode.replace` / `parentNode.remove` from above
    // Doesn't actually happen until the commit phase, which allows us to continue to use

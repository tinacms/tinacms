...
export const isAuthorized = async (req) => {
    ...
    if (typeof clientID === 'string' && typeof token === 'string') {
        try {
            ...
            const tinaCloudRes = await fetch(`https://identity.tinajs.io/v2/apps/${clientID}/currentUser`, {
                headers: new Headers({
    ...
    !token &&
        console.error('A authorization header was not found. Please use the cms.api.tina.fetchWithToken function on the frontend');
    return undefined;
};

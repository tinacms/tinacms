...
describe('filesystem bridge', () => {
  describe.each([ABSOLUTE_CONTENT_PATH, './tests/filesystem-bridge/content'])('globs markdown files', async (rootPath) => {
    test(`globs markdown files with path ${rootPath}`, async () => {
      ...
      expect(foundItems).toEqual(expect.arrayContaining(['file1.md', 'file2.md']));
    })
  })

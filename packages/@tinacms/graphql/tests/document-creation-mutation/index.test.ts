...
it('creates document with all field types and validates bridge writes', async () => {
  ...
  await expect(format(result)).toMatchFileSnapshot('createDocument-response.json');

  const newDocWrite = bridge.getWrite('posts/comprehensive-post.md');
  expect(newDocWrite).toBeDefined();
  await expect(newDocWrite).toMatchFileSnapshot('comprehensive-post-content.md');
});
...
it('creates document using createCollection endpoint', async () => {
  ...
  await expect(format(result)).toMatchFileSnapshot('createCollection-response.json');

  const newDocWrite = bridge.getWrite('posts/create-collection-post.md');
  expect(newDocWrite).toBeDefined();
  await expect(newDocWrite).toMatchFileSnapshot('create-collection-post-content.md');
});
...
it('validates immediate document availability after creation', async () => {
  ...
  await expect(format(queryResult)).toMatchFileSnapshot('getDocument-response.json');
});

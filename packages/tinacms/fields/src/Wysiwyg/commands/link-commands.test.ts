...
let { forDoc, shouldNotRun, doc, p, link, text } = new PMTestHarness(defaultBlockSchema)
...
describe("stopEditingLink", () => {
  ...
  it("should set editing to '' on all links", () => {
    forDoc(doc(p(link("one", { href: "/one", editing: "editing" }), link("two", { href: "/two", editing: "editing" }))))
      .apply(stopEditingLink)
      .expect(doc(p(link("one", { href: "/one", editing: "" }), link("two", { href: "/two", editing: "" }))))
  })

  it("should set editing creating to '' on a link", () => {
    forDoc(doc(p(link("test", { href: "/test", editing: "editing", creating: "creating" }))))
      .apply(stopEditingLink)
      .expect(doc(p(link("test", { href: "/test", editing: "", creating: "" }))))
  })
  ...
  it("should set editing and creating to '' on all links", () => {
    forDoc(
      doc(
        p(
          link("one", { href: "/one", editing: "editing", creating: "creating" }),
...
describe("removeLinkBeingEdited", () => {
  ...
  it("should remove a link being edited no matter where it is", () => {
    forDoc(doc(p(link("one", { href: "/one" }), link("two", { href: "/two", editing: "editing" }))))
      .apply(removeLinkBeingEdited)
...
describe("updateLinkBeingEdited", () => {
  ...
  it("should set title on link being edited", () => {
    forDoc(doc(p(link("one", { href: "/one", editing: "editing" }))))
      .apply(updateLinkBeingEdited, { href: "/one", editing: "editing", creating: "", title: "Link Title" })
      .expect(doc(p(link("one", { href: "/one", editing: "editing", title: "Link Title" }))))
  })
})

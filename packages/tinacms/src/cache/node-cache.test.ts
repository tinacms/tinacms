...
type MockPath = ReturnType<
  typeof createUnixMockPath | typeof createWindowsMockPath
>;
type MockOs = { tmpdir: ReturnType<typeof vi.fn> };

const callMakeCacheDir = (
  dir: string,
  mockFs: MockFs,
  mockPath: MockPath,
  mockOs: MockOs = {} as MockOs
) => makeCacheDir(dir, mockFs as any, mockPath as any, mockOs as any);

describe('makeCacheDir', () => {
  describe('Unix/Linux/macOS', () => {
    it('should return the original directory if the root directory exists', () => {
      const mockPath = createUnixMockPath();
      const mockFs = createMockFs(true);

      const dir = '/root/.cache/1234';
      const result = callMakeCacheDir(dir, mockFs, mockPath);
    ...
    it('should fall back to tmpdir if root directory does not exist', () => {
      const mockPath = createUnixMockPath();
      const mockFs = createMockFs(false);
      const mockOs = { tmpdir: vi.fn().mockReturnValue('/var') };

      const dir = '/nonexistent/.cache/1234';
      const result = callMakeCacheDir(dir, mockFs, mockPath, mockOs);
    ...
    it('should handle macOS-style paths (/Users/...)', () => {
      const mockPath = createUnixMockPath();
      const mockFs = createMockFs(true);
    ...
    it('should handle Linux-style paths (/home/...)', () => {
      const mockPath = createUnixMockPath();
      const mockFs = createMockFs(true);
  ...
  describe('Windows', () => {
    it('should handle forward-slash normalized paths', () => {
    ...
    it('should handle native backslash paths', () => {
    ...
    it('should fall back to tmpdir when drive does not exist', () => {
      const mockPath = createWindowsMockPath(true);
      const mockFs = createMockFs(false);
      const mockOs = {
        tmpdir: vi
  ...
  describe('error handling', () => {
    it('should return null and log warning if mkdirSync fails', () => {
      const mockPath = createUnixMockPath();

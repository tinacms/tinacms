...
import { Modal, PopupModal, ModalHeader, ModalBody, ModalActions } from '../react-modals';
...
export const FormBuilder: FC<FormBuilderProps> = ({ form, onPristineChange, ...rest }) => {
  const cms = useCMS();
  ...
  return (
    <FinalForm key={tinaForm.id} form={tinaForm.finalForm} onSubmit={tinaForm.onSubmit}>
      {({
        handleSubmit,
        pristine,
        invalid,
        ...
        const canSubmit =
          !pristine && !submitting && !hasValidationErrors && !(invalid && !dirtySinceLastSubmit);
                ...
                <div className="relative flex-none w-full h-16 px-12 bg-white border-t border-gray-100 flex items-center justify-end">
                  <div className="flex-1 w-full justify-end gap-2	flex items-center max-w-form">
                    ...
                    {tinaForm.actions.length > 0 && (
                      <FormActionMenu form={tinaForm as any} actions={tinaForm.actions} />
                    )}
                  </div>
                </div>
              )}
...
const Emoji = ({ className = '', ...props }) => (
  <span className={`text-[40px] leading-none inline-block ${className}`} {...props} />
);
...
export const CreateBranchModel = ({
  ...
  const onCreateBranch = (newBranchName) => {
    ...
    localStorage.setItem('tina.createBranchState.values', JSON.stringify(values));
    ...
    <Modal>
      <PopupModal>
        <ModalHeader close={close}>
          <BiGitBranch className="w-6 h-auto mr-1 text-blue-500 opacity-70" /> Create Branch
        </ModalHeader>
        <ModalBody padded={true}>
          <p className="text-lg text-gray-700 font-bold mb-2">This content is protected 🚧</p>
          <p className="text-sm text-gray-700 mb-4">
            To make changes, you need to create a copy then get it approved and merged for it to go
            live.
          </p>
          <PrefixedTextField

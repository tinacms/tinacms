    ...
    <div className='w-full max-w-full' {...getRootProps()}>
      ...
      {value ? (
        loading ? (
          ...
          >
            <button
              className='flex-shrink min-w-0 focus-within:shadow-outline focus-within:border-blue-500 rounded outline-none overflow-visible cursor-pointer border-none hover:ring-2 hover:ring-orange-500 transition ease-out duration-100'
              onClick={onClick}
              ref={ref}
            >
              {isImage(src) ? (

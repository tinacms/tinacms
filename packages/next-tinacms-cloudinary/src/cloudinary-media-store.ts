...
export const createCloudinaryMediaStore = (
  options: TinaCloudCloudinaryOptions
) =>
  class CloudinaryMediaStore implements MediaStore {
    ...
    async persist(media: MediaUploadOptions[]): Promise<Media[]> {
      ...
      for (const item of media) {
        ...
        const res = await this.fetchFunction(options.baseURL, {
          method: 'POST',
          body: formData,
        })
    ...
    async delete(media: Media) {
      await this.fetchFunction(options.baseURL + encodeURIComponent(media.id), {
        method: 'DELETE',
      })
    }

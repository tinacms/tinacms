...
for file in `find $PWD \( -name .yarn -prune \) -or \( -name "*.js" -or -name "*.ts" -or -name "*.tsx" \) -not -path "*/examples/*" -not -path "*/build/*" -not -path "*/.yarn/*" -not -path "*/deprecated-packages/*" -not -path "*/node_modules/*" -print -type f`

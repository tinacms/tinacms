...
- **Schemas:** Shared fields in `tina/schemas/shared-fields.ts` (e.g., `dateFieldSchemas`, `makeSlugify`, `actionsFieldSchema`, `colorFieldSchema`). Block schemas in `tina/schemas/blocks.ts`, icon schema in `tina/schemas/icon.ts`. This differs from Next.js where schemas are co-located with components — extracting avoids messy cross-directory imports between `tina/` and `src/`.

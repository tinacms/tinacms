...
export const Icon = ({
  data,
  className = '',
  parentThemeColor = 'blue',
  ...rest
}: IconProps & { parentThemeColor?: string }) => {
  const rawColor = data.color || parentThemeColor;
  const color = rawColor === 'primary' ? parentThemeColor : rawColor;
  ...
  const IconComponent =
    IconOptions[data.name as keyof typeof IconOptions] || null;

...
export default async function PostsPage() {
  ...
  const edgesWithFormattedDates = (allPosts.data.postConnection.edges ?? []).map((edge: any) => {
    if (!edge?.node?.date) return edge
    return { ...edge, node: { ...edge.node, formattedDate: formatDate(edge.node.date) } }
  })
  ...
  return (
    <PostsClientPage query={allPosts.query} variables={allPosts.variables} data={enrichedData} />
  )
}

...
import { buttonColorClasses, invertedButtonColorClasses, linkButtonColorClasses } from '@/lib/utils'
            ...
            element = (
              <Link 
                ...
                className={`z-10 relative inline-flex items-center px-7 py-3 font-semibold text-lg transition duration-150 ease-out rounded-lg transform focus:shadow-outline focus:outline-none focus:ring-2 ring-offset-current ring-offset-2 whitespace-nowrap ${
                  parentColor === 'primary'
                    ? invertedButtonColorClasses[theme.color as keyof typeof invertedButtonColorClasses]
                    : buttonColorClasses[theme.color as keyof typeof buttonColorClasses]
                }`}
              >
                {action.label}
                {action.icon && (
            ...
            element = (
              <Link 
                ...
                className={`z-10 relative inline-flex items-center px-7 py-3 font-semibold text-lg transition duration-150 ease-out ${
                  parentColor === 'primary'
                    ? linkButtonColorClasses[theme.color as keyof typeof linkButtonColorClasses]
                    : linkButtonColorClasses[theme.color as keyof typeof linkButtonColorClasses]
                }`}
              >
                {action.label}

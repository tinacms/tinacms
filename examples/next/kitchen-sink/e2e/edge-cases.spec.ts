...
test.describe('Edge Cases — Invalid Routes', () => {
  test('non-existent post should show 404 gracefully', async ({ page }) => {
    await page.goto('/posts/nonexistent-post-xyz', { waitUntil: 'networkidle' })

    // Should either show a 404 page or redirect gracefully
    // Accept either outcome, but the page should not crash
    const hasErrorMessage = await page.locator('text=404').isVisible({ timeout: 2000 }).catch(() => false)
    ...
    expect(hasErrorMessage || hasNotFound || page.url().includes('/')).toBeTruthy()
  })
...
test.describe('Edge Cases — Network Issues', () => {
  test('should show graceful error when API is slow', async ({ page, request }) => {
    ...
    if (href) {
      ...
      const navigationPromise = page.goto(href, { timeout: 30000, waitUntil: 'domcontentloaded' }).catch(() => null)
